    ...
    def timer_callback(self):
        ...
        if self.lidar_data is not None and self.lidar_data.data is True:
            ...
            if self.lane_data is None:
                ...
                car_center_point = (376, 179) # roi가 잘린 후 차량 앞 범퍼 중앙 위치 376,179

                target_slope = DMFL.calculate_slope_between_points(target_point, car_center_point)
                
                if target_slope > 0:
                    self.steering_command =  5 # 예시 속도 값 (7이 최대 조향) 
                elif target_slope < 0:
                ...
                #     self.steering_command = 0
                ###########################
            self.left_speed_command = 160  # 예시 속도 값 (255가 최대 속도)
            self.right_speed_command = 160  # 예시 속도 값 (255가 최대 속도)
